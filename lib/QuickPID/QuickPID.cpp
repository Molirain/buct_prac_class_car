...
// ================== Arduino 补丁区开始 ==================
#include "stm32h7xx_hal.h" // 引入 STM32 HAL 库

// 1. 补齐 Arduino 的 constrain 宏 (限制范围)
#ifndef constrain
#define constrain(amt,low,high) ((amt)<(low)?(low):((amt)>(high)?(high):(amt)))
#endif

// 2. 补齐 Arduino 的 micros() 函数 (获取微秒数)
// 我们用 HAL_GetTick() 获取毫秒，乘 1000 换算成微秒。
// 虽然没有真正 1 微秒的精度，但配合你 10ms (10000us) 的调度周期，绝对够用且完美兼容！
